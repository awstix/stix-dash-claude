 ...
 * admin having to remember to also update Zugriffe manually. Deliberately
 * only grants - never revokes - since taking access away automatically
 * could strip access an admin granted for an unrelated reason. Admins
 * already see every project regardless (getAccessibleProjectIds returns
 * null for them), so this only matters for non-admin accounts. */
