...
export type DirectionsPdfResult = {
  bytes: Uint8Array;
  fileName: string;
  projectLabel: string;
};

/** Builds the "Wegbeschreibung zur Baustelle" PDF for a project. Callers
 * are responsible for access control (requireProjectAccess) before calling
 * this - it does no auth checks itself, so it can be reused both by the
 * download route and by the email-sending action. */
export async function buildDirectionsPdf(
  projectId: string,
  actorName: string | null,
): Promise<DirectionsPdfResult | null> {
  ...
  if (mapImage) {
    ...
    // OSM attribution, drawn as real PDF text (not baked into the raster -
    // sharp/librsvg has no reliable font in the serverless runtime and
    // would render it as solid boxes instead of glyphs).
    const attributionText = "© OpenStreetMap-Mitwirkende";
    const attributionWidth = regular.widthOfTextAtSize(attributionText, 6.5);
    page.drawRectangle({
      color: rgb(1, 1, 1),
      height: 11,
      opacity: 0.8,
      width: attributionWidth + 8,
      x: MARGIN,
      y: y - MAP_DISPLAY_HEIGHT,
    });
  ...
  const footerText = `Erstellt am ${generatedAt}${actorName ? ` von ${actorName}` : ""}`;
  ...
  const projectLabel = [project.projectNumber, project.name].filter(Boolean).join(" · ");

  return { bytes: pdfBytes, fileName, projectLabel };
}
