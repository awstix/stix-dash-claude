...
import type { DayOffActionState } from "./actions";

const initialDayOffActionState: DayOffActionState = {
  error: null,
  errorKey: 0,
  success: false,
  successKey: 0,
...
export function DayOffActionForm({
  action,
  children,
  className,
  resetOnSuccess = false,
}: {
  action: (state: DayOffActionState, formData: FormData) => Promise<DayOffActionState>;
  children: ReactNode;
  className?: string;
  resetOnSuccess?: boolean;
}) {
  const [state, formAction, isPending] = useActionState(action, initialDayOffActionState);
  ...
  return (
    <>
      <form className={className} onSubmit={submitForm} ref={formRef}>
        <fieldset className="contents" disabled={isPending} key={resetOnSuccess ? state.successKey : 0}>
          {children}
        </fieldset>
      </form>
