...
export async function importProjects(formData: FormData) {
  ...
  for (const [rowIndex, row] of rows.entries()) {
    ...
    try {
      ...
      const data = {
        ...
        changeOrdersNet: moneyCents(rowValue(row, "Nachträge netto EUR")) ?? 0,
        ...
        contractValueNet:
          moneyCents(rowValue(row, "Auftragssumme netto EUR")) ?? 0,
        dvgw: bool(rowValue(row, "DVGW")),
        finalInvoiceCreated,
        finalInvoiceNet: finalInvoiceCreated
          ? moneyCents(rowValue(row, "Schlussrechnung netto EUR"))
          : null,
  ...
  if (reportWorkbook.SheetNames.length > 0) {
    ...
    const reportStoragePath = `project-import-reports/${reportFileName}`;
    await putFile(
      STORAGE_BUCKET,
      reportStoragePath,
